...
CHINEXT_LIMIT_REFORM_DATE = date(2020, 8, 24)
STAMP_DUTY_REFORM_DATE = date(2023, 8, 28)


def _parse_trade_date(value: date | datetime | str | None) -> date | None:
...
def price_limit_pct_for_symbol(
    symbol: str,
    trade_date: date | datetime | str | None = None,
    fallback: float = 0.10,
) -> float:
    """Return the regular daily price-limit ratio for an A-share board.

    IPO no-limit windows and historical point-in-time ST status need listing and
    security-status data that the strategy feed does not currently carry.
    """
    code = str(symbol or "").strip().upper()
    digits = code.split(".", 1)[0]
    if code.endswith(".BJ") or digits.startswith(("8", "9")):
        return 0.30
    ...
    if code.endswith(".SZ") and digits.startswith(("300", "301")):
        ...
        if parsed_date is not None and parsed_date < CHINEXT_LIMIT_REFORM_DATE:
            return float(fallback)
        return 0.20
    return float(fallback)


def stamp_duty_rate_for_date(
    trade_date: date | datetime | str | None,
    before_reform: float = 0.001,
    after_reform: float = 0.0005,
) -> float:
    """Return the sell-side stamp-duty rate effective on the trade date."""
    parsed_date = _parse_trade_date(trade_date)
    if parsed_date is not None and parsed_date >= STAMP_DUTY_REFORM_DATE:
        return float(after_reform)
    return float(before_reform)
...
class AShareCommission(bt.CommInfoBase):
    ...
    def set_trade_date(self, trade_date: date | datetime | str | None) -> None:
        self._trade_date = _parse_trade_date(trade_date)
        ...
        if size < 0:
            stamp_duty = (
                stamp_duty_rate_for_date(
                    self._trade_date,
                    before_reform=float(self.p.stamp_duty),
                    after_reform=float(self.p.stamp_duty_after_reform),
                )
                if self.p.date_aware_stamp_duty
                else float(self.p.stamp_duty)
            )
            comm += value * stamp_duty
        return comm
...
class BaseStrategy(bt.Strategy):
    ...
    def _is_limit_up(self, data) -> bool:
        if not self.p.enforce_price_limits or len(data) < 2:
            return False
        limit_pct = self._price_limit_pct(data)
        return data.close[0] >= data.close[-1] * (1 + limit_pct) * 0.999

    def _is_limit_down(self, data) -> bool:
        if not self.p.enforce_price_limits or len(data) < 2:
            return False
        limit_pct = self._price_limit_pct(data)
        return data.close[0] <= data.close[-1] * (1 - limit_pct) * 1.001

    def _price_limit_pct(self, data) -> float:
        if not self.p.board_aware_price_limits:
            return float(self.p.limit_pct)
        return price_limit_pct_for_symbol(
            self.p.symbol,
