...
function rollingMarketAmount(values,windowSize){var window=[],sum=0;return(values||[]).map(function(value){var n=Number(value);if(!isFinite(n))return null;window.push(n);sum+=n;if(window.length>windowSize)sum-=window.shift();return window.length===windowSize?Number((sum/windowSize).toFixed(2)):null;});}
...
function forecastKlineTooltip(params,d){
 ...
 [['三市总成交额',compactTushareAmount],['三市总额5日均额',compactTushareAmount],['三市总额20日均额',compactTushareAmount],['量额比',volumeAmountRatio],['成交额',compactTushareAmount],['成交量',compactNumber]].forEach(function(config){var item=byName[config[0]];if(item&&item.value!=null)rows.push(forecastTooltipRow(item.marker||'',config[0],config[1](item.value)));});
 if(isFinite(close)){
 ...
 items.forEach(function(item){if(!item||['K线','MA20','MA60','MA120','成交量','成交额','量额比','三市总成交额','三市总额5日均额','三市总额20日均额'].includes(item.seriesName)||item.value==null)return;var value=Array.isArray(item.value)?item.value[item.value.length-1]:item.value;if(value!=null&&isFinite(Number(value)))rows.push(forecastTooltipRow(item.marker||'',item.seriesName,Number(value).toFixed(2)));});
 return rows.join('');
}
function forecastKlineOption(d){
 ...
 var marketAmountMA5=rollingMarketAmount(marketAmount,5),marketAmountMA20=rollingMarketAmount(marketAmount,20);
 var series=[
