...
Usage:
    python .agents/skills/run-quant-yb/smoke.py           # quick smoke (sma_cross only)
    python .agents/skills/run-quant-yb/smoke.py --full    # all strategies + REPL
    python .agents/skills/run-quant-yb/smoke.py --symbol 600519.SH  # pick a stock
"""
...
def step_imports() -> bool:
    ...
    # Verify every production strategy loads.
    strategies = [
        "sma_cross",
        "macd_cross",
        "kdj",
...
def step_backtest(symbol: str, strategy: str = "sma_cross", extra_args: list = None) -> bool:
    ...
    trade_path = TRADES_DIR / f"{symbol}_{strategy}.csv"
...
def step_report(symbol: str, strategy: str = "sma_cross") -> bool:
    ...
    report_path = REPORTS_DIR / f"{symbol}_{strategy}.html"
...
def main():
    parser = argparse.ArgumentParser(description="Run the QuantYB cached-data smoke test")
    parser.add_argument("--full", action="store_true", help="run all strategies, compare, and REPL")
    parser.add_argument("--symbol", default="000001.SZ", help="cached A-share symbol")
    ...
    if full:
        ...
        results["backtest_breakout"] = step_backtest(symbol, "volume_platform_breakout")
        results["backtest_multi_timeframe"] = step_backtest(symbol, "multi_timeframe_volume_trend")
